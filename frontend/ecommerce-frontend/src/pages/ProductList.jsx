...
const ProductList = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        let baseUrl = `http://127.0.0.1:8000/api/products/items/`;
  ...
  if (loading) return <div className="p-20 text-center text-gray-400 text-sm italic">Searching for products...</div>;
    ...
    <div className="bg-[#F7FAFC] min-h-screen pb-10 font-sans text-[#1C1C1C] antialiased">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <nav className="hidden sm:flex items-center gap-1 py-4 text-[#8B96A5] text-[13px]">
          <span className="cursor-pointer hover:text-blue-600" onClick={handleClearAll}>Home</span> <ChevronRight size={12} />
          <span>{categoryTerm || "All Categories"}</span> 
          {searchTerm && (
            <>
              <ChevronRight size={12} />
              <span className="text-[#505050]">Search: {searchTerm}</span>
            </>
          )}
        </nav>
        ...
        <div className="flex flex-col lg:flex-row gap-5">
          ...
          `}>
            ...
            <FilterSidebarContent activeCategory={categoryTerm} navigate={navigate} />
          ...
          <main className="flex-1">
            ...
            <div className="bg-white border border-gray-200 rounded-lg px-4 py-2 mb-3 flex justify-between items-center shadow-sm">
              <div className="text-[13px] font-medium hidden sm:block">
                <span className="font-bold">{products.length}</span> items found {searchTerm && <span>for "<span className="text-blue-600">{searchTerm}</span>"</span>}
              </div>
            ...
            {/* TOP FILTER TAGS BAR */}
            {(searchTerm || (categoryTerm && categoryTerm !== 'All categories')) && (
              <div className="flex flex-wrap items-center gap-2 mb-4">
                 {searchTerm && (
                   <div className="flex items-center gap-2 border border-blue-500 bg-white px-3 py-1 rounded text-[13px] text-[#505050]">
                     Keyword: {searchTerm} <X size={14} className="text-gray-400 cursor-pointer hover:text-red-500" onClick={handleClearAll} />
                   </div>
                 )}
                 {categoryTerm && categoryTerm !== 'All categories' && (
                   <div className="flex items-center gap-2 border border-blue-500 bg-white px-3 py-1 rounded text-[13px] text-[#505050]">
                     Category: {categoryTerm} <X size={14} className="text-gray-400 cursor-pointer hover:text-red-500" onClick={handleClearAll} />
                   </div>
                 )}
                 <button onClick={handleClearAll} className="text-blue-600 text-[13px] font-semibold ml-2">Clear all filters</button>
              </div>
            )}
            ...
            <div className={viewType === 'grid' ? "grid grid-cols-2 md:grid-cols-3 gap-3" : "flex flex-col gap-3"}>
              ...
              {products.length === 0 && (
                <div className="col-span-full py-24 bg-white border border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center text-center px-6">
                  ...
                  <button 
                    ...
                    className="mt-6 bg-blue-600 text-white px-6 py-2 rounded-md font-bold hover:bg-blue-700 transition-colors"
                  >
                    View All Products
                  </button>
                </div>
            ...
            {products.length > 0 && <Pagination productsCount={products.length} />}
          </main>
        </div>
      </div>
    </div>
...
const ProductCard = ({ product, viewType, navigate }) => {
  ...
  if (isGrid) {
    return (
      <div className="bg-white border border-gray-200 rounded-md p-3 flex flex-col hover:shadow-lg transition-all relative group h-full">
        ...
        <div onClick={() => navigate(`/product/${product.id}`)} className="cursor-pointer h-40 flex items-center justify-center mb-3">
          <img src={product.image_url} alt={product.name} className="max-h-full max-w-full object-contain group-hover:scale-105 transition-transform" />
        </div>
        <div className="border-t border-gray-100 pt-3 flex flex-col flex-1">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-base font-bold text-[#1C1C1C]">${price}</span>
          ...
          <div className="flex items-center gap-1 mb-2">
            <div className="flex text-[#FF9017]">
              {[...Array(5)].map((_, i) => <Star key={i} size={11} fill={i < Math.floor(product.rating || 0) ? "#FF9017" : "none"} className="text-[#FF9017]" />)}
            </div>
            <span className="text-[#FF9017] text-[11px] font-bold">{product.rating || 0}</span>
          </div>
          ...
          <button 
            ...
            className="w-full mt-auto py-2 bg-blue-600 text-white rounded text-[12px] font-bold flex items-center justify-center gap-2 hover:bg-blue-700"
          >
    ...
    <div className="bg-white border border-gray-200 rounded-md flex flex-col md:flex-row p-4 hover:shadow-md transition-shadow">
      ...
      <div className="flex-1 px-0 md:px-6 py-2">
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="flex text-[#FF9017]">
            {[...Array(5)].map((_, i) => <Star key={i} size={14} fill={i < Math.floor(product.rating || 0) ? "#FF9017" : "none"} className="text-[#FF9017]" />)}
          </div>
          <span className="text-[#FF9017] text-[14px] font-bold">{product.rating || 0}</span>
      ...
      <div className="w-full md:w-48 flex flex-col justify-between border-t md:border-t-0 md:border-l border-gray-100 pt-4 md:pt-0 md:pl-6">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xl font-bold text-[#1C1C1C]">${price}</span>
            {product.old_price && <span className="text-[13px] text-gray-400 line-through">${product.old_price}</span>}
          </div>
        ...
        <div className="flex flex-col gap-2 mt-4">
          <button 
            onClick={() => addToCart(product)}
            className="w-full py-2 bg-blue-600 text-white rounded font-bold text-[14px] flex items-center justify-center gap-2 hover:bg-blue-700"
          >
             Add to cart
          </button>
...
const FilterSidebarContent = ({ activeCategory, navigate }) => (
  <div className="space-y-1">
    <FilterSection title="Category">
      <ul className="text-[14px] text-[#505050] space-y-2">
        <li 
          className={`cursor-pointer ${!activeCategory || activeCategory === 'All categories' ? 'text-blue-600 font-bold' : 'hover:text-blue-600'}`}
          onClick={() => navigate('/products')}
        >
          All categories
        </li>
        <li 
          className={`cursor-pointer ${activeCategory === 'Mobile accessory' ? 'text-blue-600 font-bold' : 'hover:text-blue-600'}`}
          onClick={() => navigate('/products?category=Mobile accessory')}
        >
          Mobile accessory
        </li>
        <li 
          className={`cursor-pointer ${activeCategory === 'Clothes' ? 'text-blue-600 font-bold' : 'hover:text-blue-600'}`}
